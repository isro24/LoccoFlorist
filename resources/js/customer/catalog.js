...
function OffcanvasControl(canvasId, openBtnId, closeBtnId) {
    ...
    if (!canvas || !backdrop || !openBtn || !closeBtn) {
        console.warn('Elemen Offcanvas tidak ditemukan, fungsionalitas filter mungkin rusak.');
        return;
    }
    ...
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape' && !canvas.classList.contains('translate-x-full')) {
            closeCanvas();
        }
    });
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // 1Inisialisasi Sidebar Filter
    OffcanvasControl('filterSidebar', 'openFilterSidebarBtn', 'closeFilterSidebarBtn');
